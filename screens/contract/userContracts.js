...
function UserContracts({ route, navigation }) {
  ...
  const [filter, setFilter] = useState("Unsigned");
      ...
      >
        ...
        <View style={styles.propertyDetails}>
          ...
          {item.status === "Cancelled" ? <View
            ...
            </View> : item.TenantSignature && item.LandlordSignature ? (
            <View
              style={{
                borderRadius: 5,
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            Signed ({contractData?.filter(x => x.TenantSignature && x.LandlordSignature).length})
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
        ...
        >
          ...
          >
            Unsigned ({contractData?.filter(x => !x.TenantSignature && x.status != "Cancelled" || !x.LandlordSignature && x.status != "Cancelled").length})
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          ...
          style={{
            backgroundColor:
              filter === "Cancelled" ? Colors.primaryColor : Colors.whiteColor,
        ...
        >
          ...
          >
            Cancelled ({contractData?.filter(x => x.status == "Cancelled").length})
          </Text>
        </TouchableOpacity>
      </TouchableOpacity>
    );
  ...
  const { data: selectedContract, update } = useDocument(
    selectedItem ? `contracts/${selectedItem}` : undefined
  );

  const hiddenItem = (data, rowMap) => {
    return (
      data.item.status != "Cancelled" ? <View style={styles.rowBack}>
        <TouchableOpacity
      ...
      </View> : <View style={styles.rowBack}>
        <TouchableOpacity
          style={[styles.backRightBtn, styles.backRightBtnRight]}
          onPress={async () => {
            // await setSelectedItem(data.item?.id);
        ...
        >
          <Text style={styles.backTextWhite}>Re Issue</Text>
        </TouchableOpacity>
      </View>
        ...
        data={
          contractData
            ? contractData
                .filter((x) =>
                  filter === "Signed"
                    ? x.TenantSignature && x.LandlordSignature && x.status != "Cancelled"
                    : filter === "Unsigned"
                    ? !x.TenantSignature && x.status != "Cancelled" || !x.LandlordSignature && x.status != "Cancelled"
                    : filter === "Cancelled" ? x.status == "Cancelled" : x
                )
                .filter(
                  (item) =>
...
UserContracts.navigationOptions = () => {
  return {
    header: () => null,
